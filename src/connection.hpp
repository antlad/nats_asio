...
public:
	...
	virtual status publish(string_view subject,
						   const char* raw,
						   std::size_t n,
	...
	virtual std::pair<isubscription_sptr, status> subscribe(string_view subject,
															optional<string_view> queue,
															on_message_cb cb,
															ctx c) override;
...
private:
	...
	virtual void on_message(string_view subject,
							string_view sid,
							optional<string_view> reply_to,
							std::size_t n,
							ctx c) override;
