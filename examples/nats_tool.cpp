...
class generator
	...
	: public worker {
public:
	generator(boost::asio::io_context& ioc, std::shared_ptr<spdlog::logger>& console, const nats_asio::iconnection_sptr& conn, const std::string& topic, int stats_interval, int publish_interval_ms);
...
class grubber
	: public worker {
public:
	grubber(boost::asio::io_context& ioc, std::shared_ptr<spdlog::logger>& console, int stats_interval, bool print_to_stdout);

	void on_message(nats_asio::string_view, nats_asio::optional<nats_asio::string_view>, const char* raw, std::size_t /*n*/, nats_asio::ctx);
private:
	...
	try
	{
		...
		conn = nats_asio::create_connection(ioc, console, [&](nats_asio::iconnection& /*c*/, nats_asio::ctx ctx)
		{
			...
			if (m == mode::grubber)
			{
				...
				auto r = conn->subscribe(topic, {}, std::bind(&grubber::on_message, grub_ptr.get(), _1, _2, _3, _4, _5), ctx);
...
generator::generator(boost::asio::io_context& ioc, std::shared_ptr<spdlog::logger>& console, const nats_asio::iconnection_sptr& conn, const std::string& topic, int stats_interval, int publish_interval_ms)
...
grubber::grubber(boost::asio::io_context& ioc, std::shared_ptr<spdlog::logger>& console, int stats_interval, bool print_to_stdout)
	: worker(ioc, console, stats_interval)
